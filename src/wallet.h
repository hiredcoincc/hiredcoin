...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 HRD at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zHRD send
enum ZerocoinSpendStatus {
    ZHRD_SPEND_OKAY = 0,                            // No error
    ZHRD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZHRD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZHRD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZHRD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZHRD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZHRD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZHRD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZHRD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZHRD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZHRD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZHRD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZHRD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZHRD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZHRD_SPENT_USED_ZHRD = 14,                      // Coin has already been spend
    ZHRD_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZHiredCoinAutoBackups(bool fEnabled)
    {
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == Params().MasternodeHoldCoins()) continue; // do not count MN-like outputs
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == Params().MasternodeHoldCoins()) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
