...
#include "ui_zhiredcoincontroldialog.h"
...
std::list<std::string> ZHiredCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZHiredCoinControlDialog::listMints;

ZHiredCoinControlDialog::ZHiredCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZHiredCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZHiredCoinControlDialog::~ZHiredCoinControlDialog()
{
    delete ui;
}

void ZHiredCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHiredCoinControlDialog::updateList()
{
...
void ZHiredCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHiredCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZHiredCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHiredCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZHiredCoinControlDialog::GetSelectedMints()
{
...
void ZHiredCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
