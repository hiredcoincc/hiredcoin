...
#include "zhiredcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zHRD ought to be enough for anybody." - Bill Gates, 2017
    ui->zHRDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzHRDSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzHRD->setEnabled(false);
        ui->pushButtonMintzHRD->setToolTip(tr("zHRD is currently disabled due to maintenance."));

        ui->pushButtonSpendzHRD->setEnabled(false);
        ui->pushButtonSpendzHRD->setToolTip(tr("zHRD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zHRDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzHRD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHRD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zHRD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zHRD in ") + 
...
void PrivacyDialog::on_pushButtonSpendzHRD_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHRD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zHRD
        sendzHRD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zHRD
    sendzHRD();
}

void PrivacyDialog::on_pushButtonZHiredCoinControl_clicked()
{
    ZHiredCoinControlDialog* zHiredCoinControl = new ZHiredCoinControlDialog(this);
    zHiredCoinControl->setModel(walletModel);
    zHiredCoinControl->exec();
}

void PrivacyDialog::setZHiredCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzHiredCoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzHRD()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid HiredCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zHRDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zHRD is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " HRD </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zHRD</b>";
    ...
    // use mints from zHiredCoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZHiredCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZHiredCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zhiredcoin selector in case it was used
    ZHiredCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zHiredCoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " HiredCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zHiredCoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zHRD </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zHRD "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zHRD "));
    ui->labelzHRDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzHRDSyncStatus->setVisible(fShow);
}
