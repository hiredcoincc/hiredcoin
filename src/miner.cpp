...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zHiredCoin spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zHiredCoin serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
//         pblock->vtx[0].vout[0].nValue = 500 * COIN;
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("HRDMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("HRDMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("HRDMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hiredcoin-miner");
    ...
    if (fProofOfStake && (GetTime() - nMintableLastCheck > 5 * 1)) // 5 minute check time
    {
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < (1431728309) || vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced()) {
        ...
        if(!fProofOfStake && chainActive.Height() >= Params().LAST_POW_BLOCK()) {MilliSleep(500); continue;}
        ...
        LogPrintf("Running HRDMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
